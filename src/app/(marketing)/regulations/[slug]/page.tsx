...
export async function generateMetadata({
  ...
  const page = await getPageByTypeAndSlug("regulation", slug);
  if (!page) return {};
...
export default async function RegulationDetailPage({
  ...
  const structured = page.structured_data as RegulationData;
  ...
  const pageUrl = `${SITE_URL}/regulations/${page.slug}`;

  const [relatedPages, linkedPages] = await Promise.all([
    getRelatedPages("regulation", page.slug),
  ...
  const jsonLd = {
    ...
    about: {
      "@type": "Legislation",
      name: structured.regulation_name,
    },
    publisher: {
      "@type": "Organization",
      name: "Policy Canary",
