...
export async function generateMetadata({
  ...
  const page = await getPageByTypeAndSlug("enforcement", slug);
  if (!page) return {};
...
export default async function EnforcementDetailPage({
  ...
  const structured = page.structured_data as EnforcementData;
  ...
  const pageUrl = `${SITE_URL}/enforcement/${page.slug}`;

  const [relatedPages, linkedPages] = await Promise.all([
    getRelatedPages("enforcement", page.slug),
    ...
    <div className="min-h-screen">
      ...
      <article className="max-w-6xl mx-auto px-6 pt-16 pb-16">
        ...
        <nav className="mb-8 text-sm text-text-secondary">
          ...
          <span>{structured.company_name || page.title}</span>
        </nav>
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          {structured.total_actions > 0 && (
            <span className="text-xs font-medium text-red-600 bg-red-50 px-2.5 py-1 rounded">
              {structured.total_actions} action
              {structured.total_actions !== 1 ? "s" : ""}
            </span>
          )}
