...
/**
 * Check if a newly enriched item matches any user's products.
 * Used for urgent alert triggers after enrichment.
 * Only fires for recalls, bans, and safety alerts.
 */
export async function checkItemForUrgentMatches(
  itemId: string
): Promise<Array<{ userId: string; productName: string; substances: string[] }>> {
  ...
  const urgentTypes = new Set(["recall", "ban_restriction", "safety_alert"]);
  if (!enrichment?.regulatory_action_type || !urgentTypes.has(enrichment.regulatory_action_type)) {
    return [];
  }

  const { data, error } = await adminClient.rpc("check_urgent_matches", {
    p_item_id: itemId,
  });

  if (error) {
    console.error("[matches] urgent check RPC error:", error);
    return [];
  }
  ...
  const userMap = new Map<string, { productName: string; substances: Set<string> }>();
  for (const row of rows) {
    if (!userMap.has(row.user_id)) {
      userMap.set(row.user_id, { productName: row.product_name, substances: new Set() });
    }
    userMap.get(row.user_id)!.substances.add(row.ingredient_name);
  }

  return [...userMap.entries()].map(([userId, d]) => ({
    userId,
