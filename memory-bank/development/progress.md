---
Last-Updated: 2026-04-17 (April 11-17 weekly roundup published)
Maintainer: RB
Status: Active — Launch prep + content marketing.
---
...
| **Blog Redesign: Editorial Layout + Images** | **2026-03-07** | **Shipped — Two-column article layout (sidebar TOC + share + newsletter CTA), cover image support end-to-end (Gemini generation → upload-image.mjs → Supabase Storage → next/image display), reading progress bar, featured post hero on index, 3-col grid at 1440px+, related posts, breadcrumbs, OG image tags, RSS enclosures. Migration `add_blog_cover_image_and_word_count`. Public `blog-images` storage bucket. All 3 existing posts backfilled with AI-generated hero images + word counts. Clawdbot skill updated with image workflow.** |
...
| **Anton (OpenClaw) Deployed + Autonomy Upgrade** | **2026-03-05 (deployed), 2026-03-12 (autonomy upgrade)** | **Live on Pi 5 (clawd). Autonomy upgrade: proactive-agent patterns (WAL protocol, working buffer, compaction recovery, reverse prompting, outcome tracking, business monitoring). Context-aware cron prompts — Anton evaluates business state and decides what to work on. Decision authority: autonomous for all research/analysis/drafting, needs Rashaad for publishing/outreach. `lightContext: false` so heartbeat loads full AGENTS.md.** |
| **LinkedIn Content Automation** | **2026-03-09** | **Shipped — linkedin-post skill, query-blog.mjs + mark-linkedin-promoted.mjs scripts, `linkedin_promoted_at` column on blog_posts. 2x/week cron (Mon+Wed 10AM ET) → #linkedin-drafts Discord → manual copy/paste to LinkedIn. Content flow: Friday roundup generates blog + LinkedIn copy, Monday promotes the roundup, Wednesday is fresh content.** |
...
| **Phase 2 FDA Fetchers (IA, Guidance, Regs.gov)** | **2026-03-09** | **Shipped + enriched — 3 new data sources, all 4,093 items enriched (0 errors). Import alerts: 154 (152 enriched, 2 token-limit errors). Guidance documents: 2,761 (all enriched, 37 cross-referenced, 27 verdicts). Regulations.gov: 1,178 (1,177 enriched, 1 error). Inngest daily-ingest now 7 parallel fetchers. Migration `006_add_phase2_sources`. Total DB: ~11,680 regulatory items, all enriched.** |
| **Intelligence Pages (Programmatic SEO)** | **2026-03-12** | **All 3 surfaces complete — 45 pages, ~100K words total. Migration `007_intelligence_pages.sql`. Ingredients (25, ~51,750 words): red-no-3, cbd-cannabidiol, titanium-dioxide, red-40, yellow-5, yellow-6, bha, potassium-bromate, blue-1, blue-2, green-3, propylparaben, aspartame, azodicarbonamide, sucralose, sodium-nitrite, carrageenan, talc, formaldehyde, kratom, lead, nmn, brominated-vegetable-oil, pfas, parabens. Enforcement (10, ~18,000 words): grimmway-farms, dollar-general, fresh-ready-foods-freshrealm, resers-fine-foods, albertsons, sysco, danone, fgf-brands, deep-foods, wells-enterprises. Regulations (10, ~30,000 words): mocra, food-dye-phase-out, pfas-food-contact, dietary-supplement-gmp, gras-reform, red-no-3-ban, fsma-204, section-781-hemp-cbd, front-of-pack-labeling, ultra-processed-food. Cross-links between pages via intelligence_page_links. Batch publish script: `scripts/backfill/publish-regulations.mjs`. 5 fact-check correction commits required.** |
| **Content Accuracy Verification** | **2026-03-12** | **Shipped — `verify-content.mjs` second-pass fact-checker. Gemini (different model/provider) verifies Claude-generated content. 3 layers: claim extraction + Google Search grounding, URL validation, Supabase cross-ref. All 3 content skills updated (weekly-roundup, seo-blog-post, linkedin-post). Deployed to Pi. ~$0.05-0.12/post.** |
| **IndexNow + Slack GSC Reminder** | **2026-03-12** | **Shipped — `src/lib/indexnow.ts`. Every blog/intelligence publish pings IndexNow (Bing/Yandex instant indexing) + sends Slack notification to `#notifications` reminding RB to submit URL to Google Search Console. Key file at `public/793bba...69.txt`. Env: `SLACK_WEBHOOK_SEO`.** |
| **Inngest Pipeline Orchestration (Phase 2C)** | **2026-03-04** | **Shipped (minimal) — daily-ingest cron (twice daily, 7 parallel fetchers + enrichment), enrich-batch (on-demand). Code-reviewed (2C + 4W fixed).** |
...
| **Product Classification** | **2026-03-07** | **Shipped — `src/lib/products/classify.ts`: Gemini Flash assigns `product_category_id` from 119-slug controlled vocab. Wired into POST + PATCH routes (non-blocking). Backfill script: `scripts/run-classify.ts` (`--force` to reclassify). Code-reviewed: fixed stale ingredients on PATCH, DSLD ingredient fetch on POST. Unlocks category matching path in `get_category_matches` RPC + email category filtering.** |
| **Sentry Error Monitoring** | **2026-03-08** | **Shipped — `@sentry/nextjs`, org `policy-canary`, project `policy-canary-web`. Client/server/edge configs, tunnel at `/monitoring`, 20% trace sampling, replay on errors. Source maps uploaded + deleted. Code-reviewed.** |
| **Blog API Revalidation Fix** | **2026-03-28** | **Shipped — `revalidatePath` on blog publish for both `/blog/[slug]` and `/blog` index. Previously new posts cached as 404 until ISR expired (up to 1 hour).** |
| **SEO Blog: FDA Data Integrity WLs 2026** | **2026-03-28** | **Published — `fda-data-integrity-warning-letters-2026`. ~1,600 words on Q1 2026 pharmaceutical data integrity enforcement. 5 companies fact-checked against DB enrichment data (Tentamus, Chemspec, Vedic, Patcos, A. Nelson). Gemini-generated cover image. Accuracy-reviewed: corrected count (7→5), softened editorial language, qualified unsourced estimates.** |
| **LinkedIn: ImmunityBio/Anktiva OPDP Letter** | **2026-03-28** | **Reviewed + anti-slop rewrite. Original had staccato AI cadence (short declarative sentences in sequence). Rewrote for natural rhythm: varied sentence length, merged choppy claim lists, removed "What's instructive isn't X. It's Y" AI pivot structure. Also fixed "companies we work with" → "companies in regulated industries" (no implied client relationship pre-launch). Hook: ~25% stock drop on warning letter. Source: FDA WL 725468.** |
| **Weekly Roundup Backfill: March 14-20** | **2026-03-28** | **Published — `weekly-fda-roundup-2026-03-14`. ~1,300 words covering 61 regulatory actions. Lead: Wegovy HD (7.2mg semaglutide) approved in 54 days via CNPV. Device recalls: Omnipod 5 insulin pump (18 injuries), Intuitive Surgical SureForm stapler (1 death, 4 injuries). Made Fresh Salads 14-product Listeria recall. 11 warning letters (Apothecary Pharma compounding GLP-1s, Vedic Lifesciences GLP fraud, 3 food importers). NAMs draft guidance. Gemini-generated cover image. Filled gap between March 6-13 and March 21-28 roundups.** |
| **Weekly Roundup Backfill: March 27 – April 3** | **2026-04-13** | **Published — `weekly-fda-roundup-2026-04-03` (backdated to Apr 3). Lead: Foundayo (orforglipron) — first new molecular entity approved under CNPV pilot, 50-day review, 294 days ahead of PDUFA. Also: March 31 CGMP warning letter cluster (Microbiological Testing & Consulting LLC pharma lab, Physitemp Instruments Class II devices without 510(k), Henan Lvyuan Chinese API via CVM — one from each sector on the same day), Kian Pee Wan supplement recall (undeclared dexamethasone + cyproheptadine), Tops/Christopher Ranch peeled garlic botulism risk, DHT clinical trials RFI, CDRH/CBER final PPI guidance, TB device reclassification, color additive carcinogenic solvent petition. Cowork draft → anti-slop pass (2 negative parallelisms fixed, ~7 em dashes cut, "put out"/"full-spectrum" replaced, sentence-case headers) → Gemini cover image → API publish → direct SQL backdate → re-upsert to trigger revalidate + IndexNow.** |
| **Weekly Roundup Backfill: April 4-10** | **2026-04-13** | **Published — `weekly-fda-roundup-2026-04-04` (backdated to Apr 11). 1,232 words covering 27 regulatory actions. Lead: FDA's April 7 GLP-1 sweep — 7 simultaneous warning letters against peptide vendors (Mile High, Prime Sciences, Pink Pony, Gram, PekCura, FormPour, Fantasy Face), including two on eBay. Also: Cook Medical catheter + Baxter Volara + AVID Medical device early alerts, Pure Indulgence 2DSCSA/Botox, Blaine Labs/Nalpac/Blueroot recalls, Wellcovorin NDA withdrawal, bioanalytical biomarker guidance. User-drafted; review corrected item count (20 → 27 via DB cross-check) and added 4 missing items (Blueroot Aller-C, AVID early alert, Wellcovorin, bioanalytical guidance).** |
| **SEO Blog Backfill: Import Alert 66-40** | **2026-04-13** | **Published — `fda-import-alert-66-40-drug-supplier-cgmp` (backdated to 2026-04-07 14:00Z). 1,410 words, `regulatory_trends` category. Target cluster: pharma data integrity / import alerts ("FDA import alert" 480/mo, "import alert 66-40" 50/mo). Explains DWPE mechanics, profiles 4 Q1 2026 IA 66-40 firms (Flowchem Pharma India, Patcos Cosmetics India, Yangzhou H&R Plastic China, Cosmetic Manufacturers Pty Australia — all verified against regulatory_items DB). Fact-check: 135 Q1 2026 WLs exact match, 21 WLs mention import alerts (post's "at least 13" conservative). Gemini cover image (port containers w/ HOLD tag). Published via direct Supabase upsert (to bypass API's `new Date()` on first publish) then re-POSTed to `/api/blog` to trigger `revalidatePath` + IndexNow (upsert preserves backdated `published_at`). Fills missed April 7 Tuesday SEO slot.** |
| **Weekly Roundup: April 11–17** | **2026-04-17** | **Published — `weekly-fda-roundup-2026-04-11` (live 22:51 UTC, id `8bbbc795-9dc2-46e8-a008-65ea26949336`). Lead: seven Class I device actions in three days (4/14-4/16) — three respiratory/anesthesia (Draeger Atlan A350, Philips Trilogy Evo, Percussionaire Phasitron 5), two implant/catheter (Stryker TMJ Concepts 2.6mm fossa discrepancy, Merit Medical 16F Splittable Sheath), plus two Early Alerts (Medical Action Industries Halyard Kit-Pack Cath, Owens & Minor American Contract Systems Coronary Angio Pack) sharing the same defective Medline Namic RA Syringe component. **Novel enforcement signal in Purolea Cosmetics Lab WL: FDA explicitly cited firm's use of AI to generate compliance documentation without Quality Unit review — first documented instance of this language, worth tracking across future CGMP letters.** Also covered: CDER sweep of 4 online pharmacies (Online Rx Medz, Rxgoodusa, Dry Springs, Beauty of Aztlan), 14 WLs on 4/14 total, clinical trial disclosure push (2,200+ sponsors, 29.6% of trials non-compliant), TRT idiopathic hypogonadism label-expansion signal (April 30 deadline for NDA holders), CBER NGS genome editing draft guidance (July 14 comments), Loard's Ice Cream + Revival Animal Health recalls. Anti-slop pass: zero em dashes (cut from 15+), sentence case headings, no pivot structures. Item count verified against DB (14 WLs not 11, 7 device actions not 6). Gemini cover image: idle anesthesia workstation + ventilator in storage, plastic-covered, neutral blue-grey palette.** |
| Session 2 (Remaining) | - | Pending — product detail image display |
...
| 2026-03-05 | **All FDA sectors in scope** | Data analysis showed 78% of WLs are tobacco+pharma, not food/supp/cosmetics. System already monitors all sectors (119 categories, 8 groups). Strategic thinking must not be artificially scoped to 3 sectors. Website marketing may focus verticals for GTM — that's separate from product scope. Warning letter pitch ("cheaper than a letter") is weak for food/supp/cosmetics; regulatory change monitoring + supply chain awareness is the stronger value prop. |
...
- **daily-ingest** — 7 fetcher steps run in parallel via `Promise.all` (Federal Register, openFDA enforcement, warning letters, FDA RSS, import alerts, guidance documents, regulations.gov), followed by sequential enrichment step (limit: 100). Each step catches errors internally (Inngest v3: failed step blocks all subsequent steps). Returns summary with per-source created/error counts.
