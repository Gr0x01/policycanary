...
created: 2026-03-03
last-updated: 2026-07-29 (verdict-gated weekly briefings)
...
**Just shipped (2026-07-29):** Weekly briefing spam fix — Zone 1 (YOUR PRODUCTS) of the paid briefing is now gated on the LLM verdict layer (`product_match_verdicts`: `relevant = true` AND resolution null/`watching`), the same source of truth the `/app/products` page uses. Previously it used raw substance/category overlap, so category-only matches (brand-specific recalls of other companies' products) spammed subscribers weekly with no way to act. Verdict-filtered items fall through to Zones 2/3 as one-liners ("everything shows up" preserved). Also: (a) briefing + free digest windows are now half-open `(start, end]` — items published on a Friday boundary no longer appear in two consecutive emails; (b) each Zone 1 item now has a `Review & resolve in Policy Canary →` deep link to `/app/products?product=&item=` (same as the urgent alert email) — resolving there removes the item from future briefings; (c) re-enrichment no longer deletes `product_match_verdicts` rows with a resolution set (`processor.ts` cleanup now filters `resolution IS NULL`), so user resolutions survive re-enrichment (verdict upsert only touches relevant/reasoning/evaluated_at). Verified live: gr0x01 account this week goes from 9 noise items to All Clear; 120-day window shows 2 relevant-verdict items still passing into Zone 1.
**Shipped 2026-04-17:** April 11-17 weekly roundup — `weekly-fda-roundup-2026-04-11` (published live 2026-04-17 22:51 UTC, id `8bbbc795-9dc2-46e8-a008-65ea26949336`). Lead: seven Class I device actions in three days (Draeger Atlan, Philips Trilogy Evo, Percussionaire Phasitron, Stryker TMJ, Merit Medical dialysis sheath, + two Early Alerts for convenience kits from Medical Action Industries and Owens & Minor/American Contract Systems sharing the same defective Medline Namic RA Syringe component). Also covered: CDER sweep of 4 online pharmacies (controlled substances + botulinum toxin), Purolea Cosmetics Lab WL (**novel enforcement signal: FDA explicitly cited firm's use of AI to generate compliance documentation without Quality Unit review — first time we've seen this language**), 14 total WLs on 4/14, clinical trial disclosure push (2,200+ sponsors), TRT idiopathic hypogonadism label-expansion signal (April 30 deadline), CBER NGS genome editing draft guidance (July 14 comment deadline). Gemini cover image (idle anesthesia + ventilator in storage, plastic-covered). Previous: IA 66-40 SEO blog (2026-04-13, backdated to 4/07), April 4-10 weekly roundup (2026-04-13 backfill). **Publishing pattern for backdated posts:** direct Supabase upsert with backdated `published_at`, then re-POST to `/api/blog` (upsert preserves existing date) to trigger `revalidatePath` + IndexNow.
