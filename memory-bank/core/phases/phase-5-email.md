...
COMPONENT 1: EMAIL QUERY LAYER (src/lib/email/queries.ts)
  ...
  a) getProductMatchesForSubscriber(userId: string, dateRange: {start, end}):
     ...
     SELECT ri.*, ie.summary, ie.key_regulations,
            ie.regulatory_action_type, ie.deadline,
            sp.product_name, pim.match_score, pim.match_reasons
  ...
  c) getActiveSubscribers(tier?: string):
     ...
     (All paid subscribers get product-matched emails)

  d) getTrendContext():
     Keep existing trend context query — unchanged.

  e) getUrgentItems(since: Date):
     Items with high-priority regulatory_action_type AND a product match for a subscriber.
...
COMPONENT 2: EMAIL COMPILER (src/lib/email/compiler.ts)
  ...
  async function compileEmail(type, items, subscriber?):
    Returns compiled HTML for the email.

  a) FREE WEEKLY DIGEST:
     - Input: all items from the period (generic, same for everyone)
     ...
     - Same content for ALL free subscribers (not personalized)
  ...
  d) WELCOME EMAIL:
     ...
     - Confirms product monitoring, sets expectations for delivery schedule
...
COMPONENT 3: REACT EMAIL TEMPLATES (src/lib/email/templates/)
  ...
  a) FreeDigest.tsx
     ...
     - "Your Weekly FDA Digest" subject line
  ...
  d) Welcome.tsx
     ...
     - Confirms: "You're signed up for product monitoring"
...
COMPONENT 4: SENDING SYSTEM (src/lib/email/sender.ts)
  ...
  a) sendCampaign(campaign: EmailCampaign):
     ...
     2. Get subscriber list for the tier
...
ACCEPTANCE CRITERIA:
- [ ] Free digest compiles generically (one for all free subs)
...
- **Free digest is generated once,** not per subscriber. Same content goes to all free subs.
